...
export class ReasoningKernel {
  ...
  async execute(agentId: string, query: string, context: string) {
    ...
    try {
      ...
      console.error(`[RK] Execution failed for ${rcbId}`, e);
      rcb.status = 'error';
      if (e.message?.includes('QUOTA_EXCEEDED')) {
        return "Neural Energy Depleted: You've exceeded your Gemini API quota. Please check your billing details or wait a moment before trying again.";
      }
      throw e;
    }
  }

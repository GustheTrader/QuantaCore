...
const DeepAgent: React.FC = () => {
  ...
  const latestOutput = session?.finalResult || session?.steps.map(s => s.content).join('\n') || '';

  return (
    <div className="flex h-[calc(100vh-160px)] gap-6">
      <div className="flex-1 animate-in fade-in duration-1000 max-w-6xl mx-auto space-y-16 pb-40 overflow-y-auto custom-scrollbar">

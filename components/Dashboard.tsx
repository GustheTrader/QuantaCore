...
const SMEForgeModal: React.FC<{
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/90 backdrop-blur-2xl p-6 animate-in fade-in zoom-in duration-300">
      <div className="glass-card w-full max-w-2xl rounded-[3.5rem] border-orange-500/30 overflow-hidden shadow-[0_0_100px_rgba(249,115,22,0.1)]">
        <div className="p-12 border-b border-slate-800 flex items-center justify-between">
          <div>
            <h2 className="text-4xl font-outfit font-black text-white uppercase tracking-tighter italic">SME <span className="text-orange-500">Forge</span></h2>
        ...
        <form onSubmit={handleSubmit} className="p-12 space-y-8">
          <div className="space-y-4">
            <label className="text-orange-400 text-[10px] font-black uppercase tracking-[0.3em] block ml-2">Core Designation (Name)</label>
            <input 
              required
              value={name}
              onChange={(e) => setName(e.target.value)}
              placeholder="e.g. QStrategic-X"
              className="w-full bg-slate-950 border-2 border-slate-800 rounded-2xl py-5 px-8 text-white font-bold focus:border-orange-500 transition-all outline-none"
            />
          </div>
          <div className="space-y-4">
            <label className="text-orange-400 text-[10px] font-black uppercase tracking-[0.3em] block ml-2">Archetype / Role</label>
            <input 
              required
              value={role}
          ...
          <div className="space-y-4">
            <label className="text-orange-400 text-[10px] font-black uppercase tracking-[0.3em] block ml-2">Logic Description</label>
            <textarea 
              value={desc}
              onChange={(e) => setDesc(e.target.value)}
              placeholder="Define the primary logic objective of this SME node..."
              className="w-full bg-slate-950 border-2 border-slate-800 rounded-2xl py-5 px-8 text-white font-medium text-sm focus:border-orange-500 transition-all outline-none min-h-[120px]"
            />
          </div>
          <div className="pt-6 flex space-x-4">
            ...
            <button type="submit" className="flex-[2] py-6 quanta-btn-orange text-white rounded-2xl font-black uppercase tracking-widest text-[11px] shadow-2xl shadow-orange-500/20 hover:scale-[1.02] transition-all">Begin Synthesis</button>
          </div>
        </form>
      </div>
    </div>
      ...
      <div className="glass-card w-full max-w-3xl rounded-[3rem] border-emerald-500/30 overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="flex-1 overflow-y-auto p-10 space-y-12 custom-scrollbar">
          ...
          <section className="space-y-6">
            ...
            <textarea value={localSettings.systemPromptOverride} onChange={(e) => setLocalSettings({ ...localSettings, systemPromptOverride: e.target.value })} placeholder="Inject custom behavioral logic for voice interactions..." className="w-full bg-slate-950 border border-slate-800 rounded-3xl p-8 text-slate-300 font-medium text-sm focus:border-emerald-500 transition-all min-h-[150px] custom-scrollbar" />
          </section>
        </div>
...
const SuperAgentCard: React.FC<AgentProps & { 
  ...
  return (
    <div className={`glass-card p-8 rounded-[3rem] flex flex-col h-full border transition-all duration-500 group hover:shadow-[0_0_50px_rgba(16,185,129,0.1)] ${isCustom ? 'border-orange-500/20 bg-orange-500/5' : 'border-emerald-500/20 hover:border-emerald-500/50'}`}>
      <div className="mb-8 flex items-start justify-between">
        ...
        <div className="flex space-x-2">
           <button onClick={(e) => { e.stopPropagation(); onConfigureVoice(); }} title="Configure Voice" className={`w-9 h-9 rounded-xl bg-slate-800 border border-slate-700 flex items-center justify-center text-slate-500 hover:text-white transition-all ${isCustom ? 'hover:border-orange-500 hover:text-orange-400' : 'hover:border-emerald-500 hover:text-emerald-400'}`}>
           ...
           <button onClick={(e) => { e.stopPropagation(); onVoice(); }} title="Activate Voice" className="w-9 h-9 rounded-xl bg-orange-600/10 border border-orange-500/30 flex items-center justify-center text-orange-400 hover:bg-orange-600 hover:text-white transition-all">
      ...
      <p className={`${isCustom ? 'text-orange-400' : 'text-emerald-400'} font-black text-[10px] uppercase tracking-widest mb-6`}>{subhead}</p>
      ...
      <div className="flex flex-wrap gap-2 mb-8 pt-6 border-t border-slate-800/50">
        {tags.map((t) => (
          <span key={t} className={`text-[9px] px-3 py-1.5 rounded-lg font-black uppercase tracking-widest bg-slate-900 border border-slate-800 text-slate-500 transition-colors ${isCustom ? 'group-hover:border-orange-500/30 group-hover:text-orange-400' : 'group-hover:border-emerald-500/30 group-hover:text-emerald-400'}`}>{t}</span>
        ))}
      </div>
      <button onClick={() => navigate('/chat', { state: { agent: name } })} className={`w-full py-5 text-white rounded-2xl font-black uppercase tracking-[0.2em] text-[11px] shadow-xl flex items-center justify-center space-x-3 transition-all ${isCustom ? 'quanta-btn-orange hover:shadow-orange-500/20' : 'quanta-btn-primary hover:shadow-emerald-500/20'}`}>
...
const ForgePlaceholderCard: React.FC<{ onClick: () => void }> = ({ onClick }) => (
  <button onClick={onClick} className="glass-card p-10 rounded-[3rem] flex flex-col items-center justify-center text-center border-2 border-dashed border-orange-500/20 hover:border-orange-500 hover:bg-orange-500/5 transition-all duration-500 group min-h-[400px]">
    <div className="w-20 h-20 rounded-full bg-slate-900 border-2 border-slate-800 flex items-center justify-center mb-6 group-hover:scale-110 transition-transform shadow-2xl">
      <svg className="w-10 h-10 text-orange-500 group-hover:rotate-90 transition-transform duration-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M12 4v16m8-8H4" /></svg>
    </div>
    <h3 className="font-outfit font-black text-2xl text-white mb-2 uppercase tracking-tighter">Forge New Core</h3>
    <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-6">Initialize Custom Neural Logic</p>
  </button>
);
...
const Dashboard: React.FC<DashboardProps> = ({ track, profile }) => {
  ...
  return (
    <div className="animate-in fade-in duration-1000 pb-32">
      ...
      {editingVoiceAgent && <VoiceConfigModal agent={editingVoiceAgent} settings={getVoiceSettings(editingVoiceAgent)} onSave={(s) => saveVoiceSettings(editingVoiceAgent.name, s)} onClose={() => setEditingVoiceAgent(null)} />}
      ...
      <nav className="flex items-center justify-between mb-24">
        ...
        <div className="flex items-center space-x-4">
           <button onClick={() => setIsForgeModalOpen(true)} className="px-6 py-3 rounded-2xl border border-orange-500/30 bg-orange-500/5 text-orange-400 text-[11px] font-black uppercase tracking-widest flex items-center space-x-2 hover:bg-orange-500 hover:text-white transition-all shadow-lg hover:shadow-orange-500/20">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          <div className="lg:col-span-4 space-y-4 max-h-[600px] overflow-y-auto custom-scrollbar pr-4">
            {allAgentsList.map((agent, i) => {
              ...
              return (
                <button key={agent.name} onClick={() => setSelectedMetricAgent(agent.name)} className={`w-full text-left p-6 rounded-[2rem] border transition-all duration-300 flex items-center justify-between group ${isSelected ? 'bg-emerald-500/10 border-emerald-500/50 shadow-xl shadow-emerald-500/5' : 'bg-slate-900 border-slate-800 hover:border-slate-700'}`}>
                  <div className="flex items-center space-x-4">
                  ...
                  <div className="flex items-end justify-between px-2">
                     {performanceData.map((d, i) => (
                       <div key={i} className="flex flex-col items-center space-y-2">
                          <div className={`w-3 rounded-full transition-all duration-1000 ${allAgentsList[i % (allAgentsList.length || 1)]?.name === selectedMetricAgent ? 'bg-blue-500 shadow-[0_0_15px_rgba(59,130,246,0.5)]' : 'bg-slate-800'}`} style={{ height: `${(d.sat || 0) * 0.8}px` }}></div>
                          <span className="text-[8px] font-black text-slate-600 uppercase tracking-tighter">SME{i+1}</span>
                       </div>
                     ))}
                  </div>
               </div>
      ...
      <section className="mb-32">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
          {agentsLoading ? Array.from({ length: 3 }).map((_, i) => <div key={i} className="h-96 glass-card rounded-[3rem] animate-pulse bg-slate-900/50" />) : (
            <>
              {agents.map((agent) => <SuperAgentCard key={agent.name} {...agent} onVoice={() => handleTriggerVoice(agent)} onConfigureVoice={() => setEditingVoiceAgent(agent)} />)}
              {customAgents.map((agent) => <SuperAgentCard key={agent.name} {...agent} isCustom onVoice={() => handleTriggerVoice(agent)} onConfigureVoice={() => setEditingVoiceAgent(agent)} />)}

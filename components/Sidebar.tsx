  ...
  return (
    ...
    >
      <div className="flex flex-col h-full">
        ...
        {/* Credit Indicators */}
        <div className={`px-4 py-4 space-y-4 transition-all ${!isOpen ? 'opacity-0 scale-90' : 'opacity-100 scale-100'}`}>
          <div className="p-3 rounded-xl bg-slate-900/50 border border-emerald-500/10">
            <div className="flex items-center justify-between mb-1.5">
              <span className="text-[8px] font-black text-emerald-500/80 uppercase tracking-widest">Cloud IQ</span>
              <span className="text-[9px] font-bold text-white">{credits.cloudTokens.toLocaleString()}</span>
            </div>
            <div className="h-1 w-full bg-slate-800 rounded-full overflow-hidden">
              <div className="h-full bg-emerald-500 shadow-[0_0_8px_#10b981]" style={{ width: `${(credits.cloudTokens / 10000) * 100}%` }}></div>
            </div>
          </div>
          <div className="p-3 rounded-xl bg-slate-900/50 border border-orange-500/10">
            <div className="flex items-center justify-between mb-1.5">
              <span className="text-[8px] font-black text-orange-500/80 uppercase tracking-widest">Deep Agent</span>
              <span className="text-[9px] font-bold text-white">{credits.deepAgentTokens.toLocaleString()}</span>
            </div>
            <div className="h-1 w-full bg-slate-800 rounded-full overflow-hidden">
        ...
        <div className="p-4 space-y-3 border-t border-slate-800/50">
          ...
          {/* Settings & Connectors at the absolute bottom */}

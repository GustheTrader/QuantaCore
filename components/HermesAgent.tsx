...
const HermesAgent: React.FC<HermesAgentProps> = ({ profile, onOpenChat }) => {
      ...
      <div className="max-w-5xl mx-auto pb-40 animate-in fade-in duration-700">
        <header className="mb-12 border-b border-slate-800 pb-8 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            <button 
              onClick={() => onOpenChat('Hermes Protocol')}
              className="px-6 py-3 bg-slate-900 border border-indigo-500/30 hover:border-indigo-500 text-indigo-400 rounded-xl transition-all flex items-center space-x-3"
            >
              ...
              <span className="text-[10px] font-black uppercase tracking-widest">Neural Terminal</span>
            </button>
            <div className="px-4 py-3 bg-orange-500/10 border border-orange-500/30 rounded-xl text-orange-400 text-[10px] font-black uppercase tracking-widest">
              Stable: Deep
            </div>
          </div>
        </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="flex flex-col">
            <div className="glass-card flex-1 p-8 rounded-[2.5rem] border-slate-800 bg-black flex flex-col overflow-hidden group">
              ...
              {logs.length > 0 && (
                <div className="mt-4 pt-4 border-t border-slate-800">
                  <ActionHub content={logs.join('\n')} agentName="Hermes Agent" title="Execution Log" />
                </div>
              )}
            </div>
          </div>
